...
const TheDetail = (props) => {
  ...
  const [activityDetail, setActivityDetail] = useState(
    { created_at: "", rating: "", note: "", activityTypeId: "" }
  );
  ...
  const getDetail = () => {
    if (isAuthenticated()) {
      fetch(`http://localhost:8000/activitydetail/${props.match.params.activityDetailId}`, {
        method: "GET",
        headers: {
  ...
  return (
    ...
    {/* if returned object is nested and you want to access, have to check that it is there and then if that evaulates to true, access the property of that nested object */}
      <h3>{activityDetail.activity_type && activityDetail.activity_type.name}</h3>
